...
\begin{equation}
R_{final} = \underbrace{w_z z}_{生存引导} - \underbrace{(d + \sqrt{d} + 0.03 d^3)}_{位置硬约束} - \underbrace{0.5 \|v\|}_{动能消除} + \underbrace{\text{StepBonus}}_{存活激励}
\end{equation}
...
\subsection{训练流程图示}
整个训练生命周期如图 \ref{fig:train_flow} 所示。从初始的随机探索，到初步掌握垂直平衡，再到最后实现受限空间内的定点静止，共计耗时约 300 万个仿真步长。
...
\begin{figure}[H]
    \centering
    % 此处预留训练曲线图位置
    \includegraphics[width=0.8\textwidth]{example-image-1.png}
    \caption{训练过程中 Episode Reward 的收敛趋势图}
