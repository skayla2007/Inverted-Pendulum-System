...
网络规格采用两层MLP，每层256神经元，激活函数ReLU。训练参数包括学习率$3 \times 10^{-4}$、批大小64、总步数300,000。环境扰动通过随机初始状态和外部力矩模拟。
...
在训练过程中，我们遇到了收敛慢和策略不稳定的问题。通过调整奖励函数权重，我们增加了对倾角的惩罚，减少了位移奖励。网络规格从一层128神经元扩展到两层256，以提高表达能力。学习率从$1 \times 10^{-3}$降低到$3 \times 10^{-4}$，以稳定训练。环境参数如阻尼系数和初始扰动也被优化。
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{example-image-1}
  \caption{训练奖励曲线}
  \label{fig:reward}
\end{figure}
